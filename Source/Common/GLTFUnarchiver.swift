...
public class GLTFUnarchiver {
    ...
    func setTexture( gltfNormalTextureInfo:GLTFMaterialNormalTextureInfo, to property:SCNMaterialProperty ) throws{
        try setTexture(index: gltfNormalTextureInfo.index , to: property)
        if( gltfNormalTextureInfo.extensions == nil ){
            return
        }
        let scals = gltfNormalTextureInfo.extensions?.extensions["KHR_texture_transform"] as! GLTFKHRMaterialsExtensions
        let xs = (scals.data?._scale![0])! * gltfNormalTextureInfo.scale
        let xy = (scals.data?._scale![1])! * gltfNormalTextureInfo.scale
        let mapScale = SCNMatrix4MakeScale(xs ?? 1, xy ?? 1, 1);
        property.contentsTransform = mapScale
    }
    func setTexture( gltfTextureInfo:GLTFTextureInfo, to property:SCNMaterialProperty ) throws{
        try setTexture(index: gltfTextureInfo.index , to: property)
        if( gltfTextureInfo.extensions == nil ){
            return
        }
        let scals = gltfTextureInfo.extensions?.extensions["KHR_texture_transform"] as! GLTFKHRMaterialsExtensions
        let xs = scals.data?._scale![0]
        let xy = scals.data?._scale![1]
        let mapScale = SCNMatrix4MakeScale(xs ?? 1, xy ?? 1, 1);
        property.contentsTransform = mapScale
    }
    ...
    private func loadMaterial(index: Int) throws -> SCNMaterial {
        ...
        if let pbr = glMaterial.pbrMetallicRoughness {
            ...
            if let baseTexture = pbr.baseColorTexture {
                try self.setTexture(gltfTextureInfo: baseTexture, to: material.diffuse)
                material.diffuse.mappingChannel = baseTexture.texCoord
//                SCNMatrix4 mapScale = SCNMatrix4MakeScale(mapModel.repeat.x, mapModel.repeat.y, 1);
//                // scenekit offset x 的坐标跟前端的是一致的, y 坐标跟前端是相反的, 所以 y 坐标需要处理
//                SCNMatrix4 mapOffset = SCNMatrix4MakeTranslation(mapModel.offset.x, mapModel.offset.y, 0);
//                materialProperty.contentsTransform = SCNMatrix4Mult(mapScale, mapOffset);
//                material.diffuse.contentsTransform = (baseTexture.extensions?.extensions ?? <#default value#>) as GLTFKHRMaterialsExtensions)
            ...
            if let metallicTexture = pbr.metallicRoughnessTexture {
                try self.setTexture(gltfTextureInfo: metallicTexture, to: material.metalness)
                material.metalness.mappingChannel = metallicTexture.texCoord
                
                try self.setTexture(gltfTextureInfo: metallicTexture, to: material.roughness)
        ...
        if let normalTexture = glMaterial.normalTexture {
            try self.setTexture(gltfNormalTextureInfo:normalTexture, to: material.normal)
            material.normal.mappingChannel = normalTexture.texCoord
            // TODO: - use normalTexture.scale
        }
        if let alphaTexture = glMaterial.alphaTexture {
            try self.setTexture(index: alphaTexture.index, to: material.transparent)
            material.transparent.mappingChannel = alphaTexture.texCoord
            material.transparent.textureComponents = .red
        }
