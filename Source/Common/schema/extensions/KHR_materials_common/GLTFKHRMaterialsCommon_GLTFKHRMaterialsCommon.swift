...
import Foundation
import SceneKit
struct GLTFKHRMaterialsExtensions:GLTFCodable{
    struct GLTFKHRTextureTransform:Codable{
        let _scale:[Float]?
        var scale:[Float]{
            get { return self._scale ?? [1,1] }
        }
        private enum CodingKeys: String, CodingKey {
            case _scale = "scale"
        }
    }
    let data: GLTFKHRTextureTransform?

    enum CodingKeys: String, CodingKey {
        case data = "KHR_texture_transform"
    }
